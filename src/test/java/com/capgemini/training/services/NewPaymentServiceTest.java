...
class NewPaymentServiceTest {
  ...
  void addPayment() {
    ...
    var result =
        newPaymentService.addPayment(
            PaymentRequest.builder().paymentId(12L).customerId("12").beneficiaryId("12").build());
  ...
  @Test
  void shouldThrowNotFoundWhenPaymentIdDoesExists() {
    when(paymentRepository.existsById(anyLong())).thenReturn(true);
    assertThrows(
        PaymentBadRequestException.class,
        () -> newPaymentService.addPayment(PaymentRequest.builder().paymentId(12L).build()));
  }
  ...
  void shouldThrowNotFoundWhenBeneficiaryIdDoesNotExists() {
    ...
    assertThrows(
        ...
        () ->
            newPaymentService.addPayment(
                PaymentRequest.builder().paymentId(12L).customerId("12").build()));
  }
}
